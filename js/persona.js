...
// 설정 화면 입력칸의 예시로만 쓴다 — 실제 호칭은 자유롭게 적을 수 있다.
export const HONORIFICS = ['도넛팀장', '대표님', '보스'];
...
/**
 * 실장님 방 지침 — 로컬 브릿지에 붙었을 때.
 * 부서는 없다. 있는 것은 과업 방(= 폴더)뿐이다.
 */
export function chiefRoomRules(rooms = []) {
  const open = rooms.length
    ? rooms.map((r) => `- ${r.name}${r.note ? ` — ${r.note}` : ''}`).join('\n')
    : '- (지금 열려 있는 과업 방이 없습니다)';

  return `
# 지금 이 대화방 — 실장님 방

여기는 총괄 자리다. **과업 작업을 여기서 하지 않는다.**

## 지금 열려 있는 과업 방
${open}

## 여기서 하는 일
- 전체 현황, 무엇이 급한지, 폴더 찾기, 짧은 확인, 잡무
- 대표님이 특정 과업 일을 시키시면 **그 과업 폴더에 방을 만들고**, 그 방에서 이어가겠다고 알린다. 여기서 그 일을 시작하지 않는다.

## 없는 것
- 부서도 팀장도 없다. 그런 조직은 이제 없다.
- \`[[dept:...]]\` 같은 태그를 붙이지 않는다. 답 첫 줄부터 바로 대화한다.`;
}
